...
import {
  ...
  FileText, Download, CheckCircle2, X, Loader2, AlertTriangle,
} from "lucide-react";
import Calendar from "../components/Calendar";
import { ErrorBlock, PageLoader } from "../components/States";
import { useStores } from "../api/useApi";
...
export default function Rapports() {
  ...
  const { stores, sansDonnees, defaut, error: storesError, loading: storesLoading } = useStores();
  const [store, setStore] = useState(null);

  // Même magasin par défaut que la Vue d'ensemble : générer un rapport sur une
  // boutique sans flux échouerait immédiatement.
  useEffect(() => {
    if (store || !defaut) return;
    setStore(defaut);
  }, [defaut, store]);
  ...
  const handleGenerate = async () => {
    setGenerating(true);
    try {
      // Les données viennent du backend : cet appel est asynchrone et peut
      // échouer (backend arrêté, période sans événement).
      const payload = await buildReportPayload(store, start, end);
  ...
  return (
    <div>
      ...
      {storesError && <ErrorBlock error={storesError} onRetry={() => window.location.reload()} />}
      {!storesError && storesLoading && <PageLoader label="Chargement des magasins…" />}
        ...
        <div className="report-form-row">
          ...
          <div className="report-field">
            ...
            <div className="popover-wrap" ref={storeRef}>
              ...
              >
                {store ?? (storesLoading ? "Chargement…" : "Aucun magasin")}
                <ChevronDown size={14} />
              </button>
              {storeOpen && (
                <div className="popover store-list left">
                  {stores.map((s) => (
                    ...
                    >
                      <Store size={14} />
                      {s}
                      {sansDonnees.has(s) && <span className="store-flag">aucun flux</span>}
          ...
          <button className="btn-generate" onClick={handleGenerate} disabled={generating || !store}>
            {generating
      ...
      {toast && (
        <div className={`toast ${toast.type}`}>
          {toast.type === "ok"
            ...
            : <AlertTriangle size={22} className="toast-icon" />}

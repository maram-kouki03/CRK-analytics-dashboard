...
import {
  Trophy, Handshake, Globe, Users,
} from "lucide-react";
import Topbar from "../components/Topbar";
import { lineColors } from "../data/mockData";
import { getComparisonData } from "../data/rangeData";

const bestIcons = { trophy: Trophy, handshake: Handshake, globe: Globe, users: Users };
...
export default function Comparaison({ rangeStart, rangeEnd }) {
  const { magasins, bestCards, conclusions, evolutionJours } = useMemo(
    () => getComparisonData(rangeStart, rangeEnd),
    [rangeStart, rangeEnd]
  );
          ...
          <table className="perf">
            <thead>
              <tr>
                ...
                <th>Taux PEC</th><th>Vendeurs actifs</th>
              </tr>
            </thead>
            <tbody>
              {magasins.map((m) => (
                <tr key={m.nom}>
                  ...
                  <td>{m.vendeurs.toFixed(1).replace(".", ",")}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div className="compare-grid-2">
        <div className="card">
          ...
          <ResponsiveContainer width="100%" height={280}>
            <LineChart data={evolutionJours} margin={{ top: 5, right: 5, left: -25, bottom: 0 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#f0e6df" vertical={false} />
